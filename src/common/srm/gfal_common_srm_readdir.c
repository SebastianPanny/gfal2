...
struct dirent* gfal_srm_readdir_internal(catalog_handle ch, gfal_srm_opendir_handle oh, int nb_files, GError** err){
	g_return_val_err_if_fail(ch && oh , NULL, err, "[gfal_srmv2_opendir_internal] invaldi args");
	GError* tmp_err=NULL;
	struct dirent* resu = NULL;
	...
	if(ret >=0){
		...
		if(srmv2_mdstatuses[0].status != 0){
			...
			for(i=0 ; i < srmv2_mdstatuses[0].nbsubpaths; ++i){
				if(i==0){
					resu = gfal_srm_readdir_convert_result(ch, &srmv2_mdstatuses[0].subpaths[i], &oh->current_readdir, &tmp_err);
					resu->d_off = oh->dir_offset;
				}
				//cache system
			}
		}
...
struct dirent* gfal_srm_readdirG(catalog_handle ch, gfal_file_handle fh, GError** err){
	...
	if(fh != NULL){
		...
		ret = gfal_srm_readdir_internal(ch, oh, 100, &tmp_err);	
		oh->dir_offset += 1;	
	}else{
