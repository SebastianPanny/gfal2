...
static int gfal_voms_init(struct vomsdata **vd, GError **err){
    ...
    if( (*vd = VOMS_Init ("", "")) ==NULL){
		gfal_create_GError(err, EINVAL, " [parse_vomsdataG][VOMS_Init] VOMS_Init failed, \
	...
	if( !VOMS_SetVerificationType (VERIFY_NONE, *vd, &error)){
        VOMS_ErrorMessage (*vd, error, errmsg, GFAL_ERRMSG_LEN);	
 		gfal_create_GError(err, EINVAL, "[parse_vomsdataG][VOMS_SetVerificationType] error : %s", errmsg);      
 		VOMS_Destroy(*vd); 	
		return -2;
	}	
	if( !VOMS_RetrieveFromProxy (RECURSE_CHAIN, *vd, &error)){
        VOMS_ErrorMessage (*vd, error, errmsg, GFAL_ERRMSG_LEN);	
 		gfal_create_GError(err, EINVAL, "[parse_vomsdataG][VOMS_RetrieveFromProxy] error : %s", errmsg);      
 		VOMS_Destroy(*vd); 	
		return -3;			
	}
	if( !(*vd)->data || !(*vd)->data[0]) {
            gfal_create_GError(err, EINVAL, "[gfal_parse_vomsdata] Unable to get VOMS info from the proxy (Memory problem?)");
            return (-4);
    }	
    return 0;	
}
...
int gfal_parse_vomsdataG (GError ** err){
	...
	gfal_voms_info->fqan= g_list_alloc();
    ...
    for (i = 0; vd->data[0]->fqan[i] != NULL; ++i) {
            ...
            if ((str = strndup (vd->data[0]->fqan[i],2048)) == NULL){
 				fal_create_GError(err, EINVAL, "[parse_vomsdataG] fqan value retrived from VOMS invalid");      
				VOMS_Destroy(vd); 					
                return (-1);
			}
			if(*str = '/'){
 				fal_create_GError(err, EINVAL, "[parse_vomsdataG] fqan first char retrived from VOMS invalid");      
				VOMS_Destroy(vd); 					
                return (-1);				
			}
...
char *gfal_get_userdnG (GError ** err){
	GError* tmp_err=NULL;
	if( gfal_parse_vomsdata(&err) != 0){// obtain from the singleton
...
GList* gfal_get_fqanG (GError** err){
	GError* tmp_err=NULL;	
	if(gfal_parse_vomsdata (&tmp_err) != 0){	// obtain from the singleton
