...
void test_access_posix_guid_exist()
{
	...
	assert_true_with_message(ret==0 && gfal_posix_code_error()==0 && errno == 0, " must be a valid access to the guid %s", strerror(errno));

	ret = gfal_access(TEST_GUID_NOEXIST_ACCESS, F_OK);
	assert_true_with_message(ret == -1 && errno == ENOENT, "must be a non exist guid %s", strerror(errno));
